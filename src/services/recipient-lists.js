...
class RecipientListManager {
    ...
    generateId() {
        return `list_${Date.now()}_${require('crypto').randomBytes(4).toString('hex')}`;
    }
    ...
    // Resolve a list file path, guaranteeing it stays inside listsDir.
    // Defense-in-depth on top of isValidId validation at every entry point.
    _listFilePath(listId) {
        const base = path.resolve(this.listsDir);
        const resolved = path.resolve(base, `${listId}.json`);
        if (!resolved.startsWith(base + path.sep)) {
            throw new Error('Invalid list ID: path escapes storage directory');
        }
        return resolved;
    }
    ...
    saveList(list) {
        ...
        const filePath = this._listFilePath(list.id);
    ...
    loadList(listId) {
        ...
        try {
            const filePath = this._listFilePath(listId);
    ...
    createList(data) {
        ...
        list.recipients = rawRecipients.map(recipient => this.validateRecipient(recipient)).filter(Boolean);
