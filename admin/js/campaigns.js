...
async function loadUserInfo() {
    try {
        ...
        if (currentUser.role === 'admin') {
            const uNav = document.getElementById('usersNavItem') || document.getElementById('nav-users');
            const aNav = document.getElementById('activitiesNavItem') || document.getElementById('nav-activities');
...
async function loadSessions() {
    try {
        ...
        sessions = Array.isArray(response.data) ? response.data : (response.data.data || []);
...
async function loadCampaigns() {
    try {
        ...
        campaigns = Array.isArray(response.data) ? response.data : (response.data.data || []);
        filterCampaigns();
    } catch (error) {
...
// Display campaigns
function displayCampaigns(campaignList = campaigns) {
...
function createCampaignCard(campaign) {
    const statusBadges = {
        draft: '<span class="campaign-badge draft"><i class="bi bi-pencil-fill me-1"></i>Draft</span>',
        ready: '<span class="campaign-badge ready"><i class="bi bi-clock-fill me-1"></i>Ready</span>',
        scheduled: '<span class="campaign-badge scheduled"><i class="bi bi-calendar-event-fill me-1"></i>Scheduled</span>',
        sending: '<span class="campaign-badge sending"><i class="bi bi-arrow-repeat spin me-1"></i>Sending</span>',
        paused: '<span class="campaign-badge paused"><i class="bi bi-pause-circle-fill me-1"></i>Paused</span>',
        completed: '<span class="campaign-badge completed"><i class="bi bi-check-circle-fill me-1"></i>Completed</span>'
    };

    const statusBadge = statusBadges[campaign.status] || `<span class="campaign-badge draft">${escapeHtml(campaign.status)}</span>`;
    const total = campaign.statistics?.total || campaign.recipients?.length || 0;
    const sent = campaign.statistics?.sent || 0;
    const failed = campaign.statistics?.failed || 0;
    const progress = total > 0 ? Math.round(((sent + failed) / total) * 100) : 0;
    const createdDate = new Date(campaign.createdAt).toLocaleDateString([], { month: 'short', day: 'numeric', year: 'numeric' });
    const msgType = campaign.message?.type ? campaign.message.type.toUpperCase() : 'TEXT';
        ...
        <div class="col-md-6 col-lg-4">
            <div class="campaign-card">
                <div class="campaign-card-header">
                    <div>
                        <h5 class="campaign-title" title="${escapeHtml(campaign.name)}">${escapeHtml(campaign.name)}</h5>
                        <div class="campaign-meta">
                            <span><i class="bi bi-calendar3 me-1"></i>${createdDate}</span>
                            <span>•</span>
                            <span class="badge bg-secondary-subtle text-secondary border px-2 py-1"><i class="bi bi-whatsapp me-1 text-success"></i>${escapeHtml(campaign.sessionId || 'Default')}</span>
                            <span>•</span>
                            <span class="badge bg-light text-dark border px-2 py-1">${msgType}</span>
                        </div>
                    </div>
                    <div>${statusBadge}</div>
                </div>
                ...
                <div class="campaign-body">
                    <div class="campaign-stats-grid">
                        ...
                        <div class="stat-item">
                            <div class="stat-val text-success">${sent}</div>
                            <div class="stat-lbl">Sent</div>
                        </div>
                        <div class="stat-item">
                            <div class="stat-val ${failed > 0 ? 'text-danger' : 'text-muted'}">${failed}</div>
                ...
                <div class="campaign-card-footer">
                    <div class="d-flex gap-2">
                        <button class="btn btn-sm btn-outline-primary fw-bold" onclick="viewCampaign('${campaign.id}')">
                            <i class="bi bi-eye-fill me-1"></i> View
                        </button>
                        ${campaign.status === 'draft' ? `
                            <button class="btn btn-sm btn-outline-secondary fw-bold" onclick="editCampaign('${campaign.id}')">
                                <i class="bi bi-pencil me-1"></i> Edit
                            </button>
                        ` : ''}
                        ${campaign.status === 'sending' ? `
                            <button class="btn btn-sm btn-warning fw-bold text-dark" onclick="pauseCampaign('${campaign.id}')">
                                <i class="bi bi-pause-fill me-1"></i> Pause
                            </button>
                        ` : ''}
                        ${campaign.status === 'paused' ? `
                            <button class="btn btn-sm btn-success fw-bold" onclick="resumeCampaign('${campaign.id}')">
                                <i class="bi bi-play-fill me-1"></i> Resume
                            </button>
                        ` : ''}
                        ${campaign.status === 'completed' && failed > 0 ? `
                            <button class="btn btn-sm btn-outline-warning fw-bold" onclick="retryCampaign('${campaign.id}')">
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end shadow-sm">
                            ${(campaign.status === 'draft' || campaign.status === 'ready') ? `
                                <li><a class="dropdown-item text-success fw-bold" href="#" onclick="sendCampaign('${campaign.id}')"><i class="bi bi-send-fill me-2"></i> Launch Now</a></li>
                                <li><hr class="dropdown-divider"></li>
                            ` : ''}
                            <li><a class="dropdown-item" href="#" onclick="cloneCampaign('${campaign.id}')"><i class="bi bi-files me-2 text-primary"></i> Clone Campaign</a></li>
                            ${(currentUser && (currentUser.role === 'admin' || campaign.createdBy === currentUser.email)) ? `
                                <li><hr class="dropdown-divider"></li>
                                <li><a class="dropdown-item text-danger fw-bold" href="#" onclick="deleteCampaign('${campaign.id}')"><i class="bi bi-trash-fill me-2"></i> Delete</a></li>
                            ` : ''}
                        </ul>
                    </div>
                </div>
...
function resetRecipientSelection() {
    ...
    const selectedRecipientsBody = document.getElementById('selectedRecipientsBody');
    ...
    if (selectedRecipientsBody) selectedRecipientsBody.innerHTML = '';
...
async function handleCSVUpload(event) {
    ...
    try {
        ...
        if (result.success) {
            ...
            showCSVErrors(result.errors || ['The CSV could not be processed.']);
        ...
        if (error.response?.status === 401) {
            ...
            showCSVErrors([error.response?.data?.message || error.message]);
...
function showCSVErrors(errors) {
    ...
    const csvErrors = document.getElementById('csvErrors');
    csvErrors.innerHTML = `<strong>Errors found:</strong><br>${messages.map(error => escapeHtml(String(error))).join('<br>')}`;
