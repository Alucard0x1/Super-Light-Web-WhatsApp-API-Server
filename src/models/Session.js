...
class Session {
    ...
    static create(sessionId, ownerEmail = null) {
        ...
        stmt.run(sessionId, validOwnerEmail, token);
    ...
    static syncWithFilesystem() {
        ...
        const adminUser = db.prepare('SELECT email FROM users WHERE email = ?').get('admin@localhost');
        const defaultOwner = adminUser ? adminUser.email : null;
        ...
        for (const sessionId of directories) {
            ...
            if (!exists) {
                const token = crypto.randomUUID();
                insertStmt.run(sessionId, defaultOwner, token);
                addedCount++;
