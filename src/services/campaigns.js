...
class CampaignManager {
    ...
    // Resolve a path inside a storage directory, rejecting anything that
    // escapes it. Defense-in-depth on top of isValidId validation: even if a
    // caller forgets to validate, a crafted ID can never reach fs as a
    // traversal path.
    _containedPath(baseDir, id, label) {
        const base = path.resolve(baseDir);
        const resolved = path.resolve(base, id);
        if (!resolved.startsWith(base + path.sep)) {
            throw new Error(`Invalid ${label} ID: path escapes storage directory`);
        }
        return resolved;
    }

    _campaignFilePath(campaignId) {
        return this._containedPath(this.campaignsDir, `${campaignId}.json`, 'campaign');
    }

    _campaignMediaPath(campaignId) {
        return this._containedPath(this.campaignMediaDir, campaignId, 'campaign media');
    }
    ...
    createCampaign(data) {
        ...
        let rawRecipients = data.recipients || [];
        if (typeof rawRecipients === 'string') {
            rawRecipients = rawRecipients.split(/[\r\n,]+/).map(s => s.trim()).filter(Boolean);
        }

        const recipients = rawRecipients.map(item => {
            const obj = (typeof item === 'object' && item !== null) ? item : { number: String(item) };
            return {
        ...
        const messageContent = data.messageContent || data.message?.content || (typeof data.message === 'string' ? data.message : '');
        ...
        const campaign = {
            ...
            message: {
                ...
                content: messageContent ? sanitizeHtml(messageContent, {
                    allowedTags: ['p', 'br', 'strong', 'em', 'u', 'a'],
                    allowedAttributes: {
                        'a': ['href', 'target']
                    }
                }) : '',
                mediaUrl: data.mediaUrl || data.message?.mediaUrl || null,
                mediaCaption: data.mediaCaption || data.message?.mediaCaption || null
            },
            ...
            settings: {
                delayBetweenMessages: data.settings?.delayBetweenMessages || (data.messageDelayMin ? data.messageDelayMin * 1000 : 3000),
    ...
    deleteCampaign(campaignId) {
        ...
        try {
            ...
            const mediaDir = this._campaignMediaPath(campaignId);
    ...
    processTemplate(template, recipient) {
        ...
        // Replace custom field placeholders (escape regex metacharacters in keys)
    ...
    // Update recipient status (increments retryCount on failure so failed
    // recipients are only re-picked while retryCount < maxRetries)
    ...
    getPendingRecipients(campaignId, limit = 100) {
        const campaign = this.loadCampaign(campaignId);
        if (!campaign) {
            console.log(`[Campaign ${campaignId}] Campaign not found`);
            return [];
        }
