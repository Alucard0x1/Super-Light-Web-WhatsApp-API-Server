 ...
 *
 * This SQLite-based campaign model is deprecated and should not be used.
 * The active campaign storage system uses encrypted JSON files via CampaignManager
 * (src/services/campaigns.js), which provides:
 * - AES-256-CBC encryption at rest
 * - File-based storage with proper permissions
 * - Active maintenance and API support
 *
 * DO NOT use this model for new campaign operations.
 * Use CampaignManager (src/services/campaigns.js) instead.
 *
 * The SQLite campaigns and campaign_recipients tables are retained only for
 * potential future migration use and are NOT used by any active routes.
 */

const { db } = require('../config/database');
const crypto = require('crypto');

// Deprecation warning (logged only once per process)
let deprecationLogged = false;
function logDeprecation() {
    if (!deprecationLogged) {
        console.warn('[Campaign Model] DEPRECATION WARNING: SQLite Campaign model is deprecated. Use CampaignManager (src/services/campaigns.js) instead.');
...
class Campaign {
     ...
     * @deprecated Use CampaignManager.createCampaign() instead
     */
    static create(data) {
        logDeprecation();
